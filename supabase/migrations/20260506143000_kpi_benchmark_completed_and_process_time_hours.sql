-- Tamamlanmış benchmark: kpi_monthly_actual aylık updated_at penceresi ile canlı RPC uyumsuzdu (çoğu ay 0).
-- Bu KPI, anlık toplam tamamlanmış sayısıdır (get_completed_benchmarks_count ile aynı); tüm aylar aynı anlık değeri taşır.
--
-- Ortalama araç kalite süresi (avg_quality_process_time): gün → saat (kart/rapor saat-dakika ile uyumlu).

CREATE OR REPLACE FUNCTION public.get_avg_quality_process_time()
RETURNS numeric
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COALESCE(
    ROUND(
      AVG(
        EXTRACT(EPOCH FROM (COALESCE(qi.shipped_at, qi.updated_at) - qi.quality_entry_at)) / 3600
      )::numeric,
      2
    ),
    0
  ...
  WHERE qi.quality_entry_at IS NOT NULL
    AND COALESCE(qi.shipped_at, qi.updated_at) IS NOT NULL
    AND COALESCE(qi.shipped_at, qi.updated_at) >= qi.quality_entry_at;
$$;

COMMENT ON FUNCTION public.get_avg_quality_process_time() IS 'Kaliteye girişten onaya ortalama süre (saat), tüm kayıtlar üzerinden.';
...
BEGIN
  ...
  IF p_kid = 'avg_quality_process_time' THEN
    SELECT COALESCE(
      ROUND(
        AVG(EXTRACT(EPOCH FROM (COALESCE(qi.shipped_at, qi.updated_at) - qi.quality_entry_at)) / 3600)::numeric,
        2
      ),
      0
    ) INTO v
      FROM quality_inspections qi
      WHERE qi.quality_entry_at >= m_start AND qi.quality_entry_at < m_end
  ...
  IF p_kid = 'completed_benchmarks_count' THEN
    SELECT COUNT(*)::numeric INTO v FROM benchmarks
...
UPDATE public.kpis SET unit = ' saat' WHERE auto_kpi_id = 'avg_quality_process_time';
