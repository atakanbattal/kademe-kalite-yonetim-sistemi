-- Eğitim kodları: aynı yıl içinde planlanan başlangıç tarihine göre sıra (EGT-YYYY-NNNN).
-- INSERT / UPDATE(start_date) / DELETE sonrası ilgili yıl yeniden numaralanır.
-- Önizleme: preview_training_code(plan_date, exclude_id, anchor_created_at, anchor_id)
...
BEGIN
  ...
  WITH scoped AS (
    ...
    WHERE EXTRACT(YEAR FROM COALESCE(t.start_date, (t.created_at AT TIME ZONE 'UTC')::date))::integer = p_year
  )
...
COMMENT ON FUNCTION public.renumber_training_codes_for_year(integer) IS
  'Verilen takvim yılındaki tüm eğitimleri start_date (nulls last), created_at, id sırasıyla EGT-YYYY-NNNN olarak yeniden numaralar.';

-- Yeni kayıt: aynı gündeki yeni kayıt en sonda (tüm start_date = D sayılır).
-- Düzenleme: exclude satırı hariç, (D, anchor_created_at, anchor_id) öncesindeki satır sayısı + 1
    ...
    SELECT COALESCE(
      (
        ...
        WHERE t.id <> p_exclude_id
          AND EXTRACT(YEAR FROM COALESCE(t.start_date, (t.created_at AT TIME ZONE 'UTC')::date))::integer = v_year
          AND t.start_date IS NOT NULL
          AND (
            t.start_date < p_plan_date
            OR (
              t.start_date = p_plan_date
              AND (
  ...
  ELSE
    SELECT COALESCE(
      (
        SELECT COUNT(*)::integer
        FROM public.trainings t
        WHERE EXTRACT(YEAR FROM COALESCE(t.start_date, (t.created_at AT TIME ZONE 'UTC')::date))::integer = v_year
          AND t.start_date IS NOT NULL
          AND (t.start_date < p_plan_date OR t.start_date = p_plan_date)
      ),
      0
    ) + 1 INTO v_next;
  END IF;
...
COMMENT ON FUNCTION public.preview_training_code(date, uuid, timestamptz, uuid) IS
  'Plan tarihine göre tahmini eğitim kodu; düzenlemede p_exclude_id + anchor ile mevcut satır hariç sıra.';
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    y_old := EXTRACT(YEAR FROM COALESCE(OLD.start_date, (OLD.created_at AT TIME ZONE 'UTC')::date))::integer;
  ...
  IF TG_OP = 'INSERT' THEN
    y_new := EXTRACT(YEAR FROM COALESCE(NEW.start_date, (NEW.created_at AT TIME ZONE 'UTC')::date))::integer;
...
-- Mevcut veriyi plan tarihine göre düzelt
DO $$
DECLARE
  r record;
BEGIN
  FOR r IN
    SELECT DISTINCT EXTRACT(YEAR FROM COALESCE(start_date, (created_at AT TIME ZONE 'UTC')::date))::integer AS y
    FROM public.trainings
    WHERE EXTRACT(YEAR FROM COALESCE(start_date, (created_at AT TIME ZONE 'UTC')::date))::integer BETWEEN 2000 AND 2100
  LOOP
