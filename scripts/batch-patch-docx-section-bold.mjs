#!/usr/bin/env node
/**
 * İç kaynaklı prosedür / talimat (ve benzeri) Word kaynaklarında
 * Amaç, Kapsam vb. bölüm başlıklarına bold uygular.
 *
 * Önizleme: node scripts/batch-patch-docx-section-bold.mjs --dry-run
 */
import fs from 'fs';
import path from 'path';
import { randomUUID } from 'crypto';
import { createClient } from '@supabase/supabase-js';
import { patchDocxSectionHeadingsBold, isDocxSource } from '../src/lib/docxSectionHeadingBold.js';
import {
...
function getDocxSources(attachments) {
    ...
    return attachments.filter((a) => a.role === 'source' && isDocxSource(a?.name, a?.type));
}
...
async function fetchTargetDocuments() {
    ...
    while (true) {
        ...
        for (const doc of data) {
            if (!TARGET_TYPES.has(doc.document_type)) continue;
...
async function patchOneDoc(doc) {
    ...
    for (let sourceIndex = 0; sourceIndex < doc.sources.length; sourceIndex += 1) {
        ...
        const { blob: patchedBlob, patched } = await patchDocxSectionHeadingsBold(originalBuf);
...
async function patchLocalFiles() {
    ...
    for (const dept of fs.readdirSync(FINAL_DIR)) {
        ...
        for (const typeFolder of fs.readdirSync(deptPath)) {
            if (!TARGET_TYPES.has(typeFolder.normalize('NFC'))) continue;
            ...
            for (const file of fs.readdirSync(typePath)) {
                ...
                const { blob, patched: changed } = await patchDocxSectionHeadingsBold(original);
                if (!changed) {
                    skipped += 1;
                    continue;
                }
...
async function main() {
    console.log(`Bölüm başlığı bold düzeltmesi${DRY_RUN ? ' (dry-run)' : ''}`);
